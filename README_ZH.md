...
*   技術實現：整合 `SheetJS (xlsx)` 套件進行前端資料解析，並特別針對台灣用戶加入 UTF-8 BOM 標頭 處理，確保轉出的 CSV 檔案在 Windows Excel 直接開啟時中文絕對不亂碼。
...
5.  精緻 UI 設計：採用現代感護眼的 Dark Mode 暗色系風格，支援流暢的拖曳上傳（Drag and Drop）體驗。
